...
RSpec.describe Admin::InsightsController, type: :controller do
  ...
  describe 'GET #new' do
    ...
    context 'when previous insights exist' do
      let!(:last_insight) do
        FactoryBot.create(:insight, model: 'Model', template: 'Template')
      end

      it 'copies model and template from last insight' do
        get :new, params: { info_request_id: info_request }
  ...
  describe 'POST #create' do
    let(:valid_params) do
      {
        ...
        insight: { model: 'TestModel', template: 'TestTemplate' }
      }
    end
    ...
    context 'with invalid params' do
      let(:invalid_params) do
        {
          info_request_id: info_request,
          insight: { model: nil, template: nil }
        }
      end
